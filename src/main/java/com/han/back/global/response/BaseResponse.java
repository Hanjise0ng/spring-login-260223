...
public class BaseResponse<T> {
    ...
    private BaseResponse(ApiResponseStatus status, String message, T result) {
    ...
    public static BaseResponse<Empty> successBody() {
        return new BaseResponse<>(ResponseStatus.SUCCESS, ResponseStatus.SUCCESS.getMessage(), Empty.getInstance());
    }

    public static <T> BaseResponse<T> successBody(T result) {
        return new BaseResponse<>(ResponseStatus.SUCCESS, ResponseStatus.SUCCESS.getMessage(), result);
    }

    public static BaseResponse<Empty> errorBody(ApiResponseStatus status) {
        return new BaseResponse<>(status, status.getMessage(), Empty.getInstance());
    }

    public static BaseResponse<Empty> errorBody(ApiResponseStatus status, String customMessage) {
        return new BaseResponse<>(status, customMessage, Empty.getInstance());
    }

    public static <T> BaseResponse<T> body(ApiResponseStatus status, T result) {
        return new BaseResponse<>(status, status.getMessage(), result);
    }
